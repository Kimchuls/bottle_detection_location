...
testdir = os.path.join(root, "learning_rate_exp")
outputdir = os.path.join(root, "result-for-lr.json")
_, dirs, _ = next(os.walk(testdir))

gtdict = gtaxis(os.path.join(gtdir, "waterdrop_annotations"))
allgt = len(gtdict)
outdict = {}
...
for dir in dirs:
    if (dir == 'allfile') or (dir == 'drawrec'):
        continue
    testdict = testaxis(os.path.join(testdir, dir))
    print(dir)
    maxscore, maxpos = 0, -1
    list = []
    sumiou = 0.0
    ...
    outdict[dir] = {"mIoU": round(sumiou, 4), "AP": round(ap, 4)}
